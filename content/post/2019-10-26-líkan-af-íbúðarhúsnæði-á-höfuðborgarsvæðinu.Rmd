...
# banka_dummy <- c("2008-10-01", "2011-12-01", "2013-12-01", "2014-12-01", "2015-01-01", "2015-02-01")
# hrun_dummy <- c("2008-11-01", "2008-12-01", "2009-01-01", "2009-02-01", "2009-03-01")
# 
# gogn <- gogn %>%
#   mutate(debt_dummy = case_when(date %in% as.Date(banka_dummy) ~ 1,
#                                 TRUE ~ 0),
#          hrun_dummy = case_when(date %in% as.Date(hrun_dummy) ~ 1,
#                                  TRUE ~ 0))
...
Gröfin að neðan sýna viðbragð fasteignaverðs á höfuðborgarsvæðinu vegna skells (e. shock) í fimm breytum. Bláa svæðið í kringum viðbragðsfallið er 80% öryggisbil. Ég hallast að bayesískri tölfræði og þar með er ég frekar gagnrýninn á öryggisbil. Auk þess sé ég ekki ástæðu til að nota 95% frekar en 80%.  

Það sem lesa má út úr viðbragðsföllunum er að hækkun hámarkshlutfalls leiðir til hærra fasteignaverðs og hærri vextir leiða til lægra fasteignaverðs þó með nokkurri töf (áhrifin virðast ekki marktæk fyrr en eftir um 1.5 ár). Það sem vekur athygli er mat líkansins á áhrifum aukinnar skuldsetningar. Áhrifin eru fyrst neikvæð, þ.e. hærri skuldsetningar leiðir til lægri fasteignaverðs en síðan snýst þetat við eftir um 20 mánuði. Tímaröðin fyrir skuldir heimila er með einhverjum brotum í þannig að hún tekur stökk bæði upp og niður á tímum sem fasteignaverð er að hækka um 5-10%. Það gæti orsakað þessi áhrif. Þá leiðir hærra leiguverð til hærra fasteignaverðs.  
...
## Variable importance með random forest
...
Grafið að neðan sýnir tvö líkön. Random forest og Cubist. Bæði líkönin virðast overfit-a þó það sé mun meira hjá Cubist. Gráa svæðið er sá hluti gagnana sem ekki var notaður við að tune-a líkönin.
...
ggplot(df_pred,
  ...
  labs(x = NULL, y = NULL, title = "Cubist og random forest", subtitle = "Gráa svæðið er test set")
...
vip_cubist <- vip(cubist_train) + labs(x = NULL, y = NULL, title = "Cubist")
vip_rf <- vip(rf_train) + labs(x = NULL, y = NULL, title = "Random forest")

gridExtra::grid.arrange(vip_cubist, vip_rf, ncol = 2)
...
# Lokaorð
Að ofan hef ég varið í gegnum hvaða áhrif mismunandi breytur hafa á fasteignaverð. Áhrifin eru meira og minna það sem búast mátti við. Spárnar eru áþekkar og benda til hægagangs eitthvað inn á 2020 en nokkurri hækkun eftir um 12 mánuði. Ég hef ekki enn lagt mat á gæ